...
lblName = tk.Label(topFrame, text = "Nombre:").pack(pady=20,padx=80,side=tk.LEFT)
entName = tk.Entry(topFrame,width=40)
entName.pack(side=tk.LEFT)
btnConnect = tk.Button(topFrame, text="Conectar", command=lambda : connect())
btnConnect.pack(padx=80,side=tk.LEFT)
...
def connect():
    ...
    if len(entName.get()) < 1:
        tk.messagebox.showerror(title="ERROR!!!", message="Nombre demasiado corto")
    else:
...
def connect_to_server(name):
    ...
    try:
        ...
        tk.messagebox.showerror(title="ERROR!!!", message="No se puede conectar al host: " + HOST_ADDR + " en: " + str(HOST_PORT) + " Intentar de nuevo")
